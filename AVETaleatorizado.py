...
sol=0
mejorRuta={}
QtC = [np.random.randint(10,16) for i in k]
CtC = [np.random.randint(10,16) for i in n]

# Bucle de AVET aleatorizado
while tiempo < 600:
    ...
    while temp <= T:
        ...
        np.random.shuffle(k) # Aleatorización orden de los vehículos           
            ...
            if rutas[i][-1][1] == temp:
                    ...
                    for s in IncidSucc[pos]:
                        if s[0] not in ocupados or s == Suc[pos]:
                            ...
                            # Si es el mejor candidato, se añade como único elemento de la lista
                            if Criterio > valor:
                                elegido = [s]
                                valor = Criterio
                                nbicis = [int(Nvalor*dist[(pos[0],s[0])])]
                            
                            # Si hay empate, se añaden a una lista
                            elif Criterio == valor:
                                elegido.append(s)
                                nbicis.append(int(Nvalor*dist[(pos[0],s[0])]))
                    
                    # Se escoge el mejor candidato (aleatoriamente en caso de empate)          
                    aleat = np.random.randint(0,len(elegido))
                    s=elegido[aleat]
                    if s == 'dum':
                        pass
                    elif tipo[s[0]]==2: # est de recogida, dem < 0
                        cantidad[i] = -nbicis[aleat]
                    elif tipo[s[0]]==1:
                        cantidad[i] = nbicis[aleat]
                    rutas[i].append(s)
                    ocupados.append(s[0])
        ...
        temp+=1
    # La mejor Fobj se inicializa con el primer resultado de AVET
    if sol == 0:
        mejorFobj = Fobj
    # Se guarda la mejor Fobj y su ruta
    elif Fobj < mejorFobj:
        mejorFobj = Fobj
        mejorRuta = rutas
    end = time.time()
    sol +=1
    print('AVET',sol,Fobj,end - start)
    tiempo = end-start